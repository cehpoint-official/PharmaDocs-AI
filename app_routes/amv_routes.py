...
def create_amv_form():
    ...
    if request.method == 'POST':
        try:
            ...
            for om_id in selected_other_ids:
                other = OtherMaterial.query.filter_by(id=om_id).first()
                if other:
    ...
    other_materials_list = OtherMaterial.query.filter_by(company_id=company_id).all()
...
def view_amv(document_id):
    ...
    user = User.query.get(session['user_id'])
    return render_template('view_amv.html', user=user, document=document, metadata=metadata, amv_details=amv_details)
...
def list_amv_documents():
    ...
    user = User.query.get(session['user_id'])
    return render_template('amv_list.html', user=user, documents=documents)
...
def manage_reagents():
    ...
    return render_template('manage_reagents.html', reagents=reagents, user=user)
...
def amv_verification_protocol_page():
    ...
    return render_template('amv_verification_protocol.html',
                           ...
                           other_materials_list=other_materials_list,
                           reagents_list=reagents_list,
                           reference_products_list=references_list, # Changed references_list to reference_products_list
                           user=user,
                           companies=companies)
...
def generate_verification_protocol():
    ...
    try:
        ...
        print(f"🔧 Selected Other IDs: {selected_other_ids}")
        ...
        selected_other = []
        for other_id in selected_other_ids:
            if other_id:
                other = OtherMaterial.query.filter_by(id=int(other_id)).first()
                if other:
                    selected_other.append({
                        'id': other.id,
                        'name': other.name,
                        'characteristics': other.characteristics or ''
                    })
        ...
        protocol_data = {
            ...
            'selected_other_materials_json': json.dumps(selected_other),
...
def protocol_generator_page():
    if 'user_id' not in session:
        return redirect(url_for('auth.login'))
    user = User.query.get(session['user_id'])
    return render_template('protocol_generator.html', user=user)
...
def list_amv_verification_documents():
    ...
    return render_template('amv_verification_list.html', documents=documents, user=user)
