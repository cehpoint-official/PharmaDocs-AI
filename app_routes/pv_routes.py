...
def generate_pvr(template_id):
    ...
    try:
        ...
        # -------------------------------
        # Collect Equipment Information (from form)
        # -------------------------------
        equipment_list = []

        for i in range(1, 6):
            equipment_name = request.form.get(f"equipment_name_{i}")
            if equipment_name:
                equipment_list.append({
                    "equipment_id": request.form.get(f"equipment_id_{i}", ""),
                    "equipment_name": equipment_name,
                    "equipment_type": request.form.get(f"equipment_type_{i}", ""),
                    "qualification_status": request.form.get(f"equipment_qualification_{i}", ""),
                    "location": request.form.get(f"equipment_location_{i}", "")
                })

        # -------------------------------
        # Collect Materials / Reagents Information (from form)
        # -------------------------------
        materials_list = []
        ...
        for i in range(1, 6):
            ...
            if material_name:
                materials_list.append({
                    "material_type": request.form.get(f"material_type_{i}", ""),
                    "material_name": material_name,
                    "specification": request.form.get(f"material_spec_{i}", ""),
                    "quantity": request.form.get(f"material_qty_{i}", "")
                })
        ...
        pvr_report.extracted = json.dumps({
            "equipment_tables": equipment_list,
            "materials_tables": materials_list
        })
        ...
        pdf_generator = ComprehensivePVRGenerator(template, batch_data, pvr_report, equipment_list=equipment_list,
