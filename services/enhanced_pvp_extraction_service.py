...
def _extract_manufacturing_params(self) -> List[Dict]:
    ...
    patterns = [
        ...
        (r'temperature.*?([\d\.]+)\s*[°c]', 'Temperature'),
        (r'pressure.*?([\d\.]+)\s*kg', 'Pressure')
    ]
    
    for pattern, param_name in patterns:
        matches = re.finditer(pattern, self.full_text, re.I)
        for m in matches:
            params.append({
